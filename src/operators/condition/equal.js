...
module.exports = function () {
  ...
  /**
   * Apply the operator 'equal' to the given measurement data, using a target value and operator configuration
   ...
   */
  OperatorEqual.applyToData = function (data, targetValue, configuration, f) {
    // Check if the target is valid
    if (targetValue === "data" || !_.startsWith(targetValue, "data.")) {
      return f(new PrettyError(500, "Invalid target '" + targetValue + "' for operator 'equal'"));
    }
    // Check if we have the 'value' attribute in the operator configuration
    if (!configuration || !configuration.value) {
      return f(new PrettyError(500, "Missing configuration attribute 'value' for operator 'equal'"));
    }
